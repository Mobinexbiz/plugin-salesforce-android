...
function createPage1UIObject() {
    sdLeftMenu = new SMF.UI.SliderDrawer();
    sdLeftMenu.width = '28%';
    sdLeftMenu.visible = true;
    sdLeftMenu.position = SMF.UI.SliderDrawerPosition.left;
    sdLeftMenu.onShow = function (e) {
        isAnimate = false;
        if (Device.deviceOS == 'iOS') {
            ...
            Pages.pgBlog.navigationItem.leftBarButtonItems = [leftItem];
        }
    };
    sdLeftMenu.onHide = function (e) {
        isAnimate = true;
        if (Device.deviceOS == 'iOS') {
            var leftItem = new SMF.UI.iOS.BarButtonItem({
                    image : "menu.png",
...
function createCategoriesDlg() {
    lblTouchStopper = new SMF.UI.Label();
    lblTouchStopper.backgroundTransparent = true;
    lblTouchStopper.width = '100%';
    lblTouchStopper.height = '100%';
    lblTouchStopper.left = 0;
    lblTouchStopper.top = 0;
    lblTouchStopper.text = '';
    lblTouchStopper.roundedEdge = 0;
    lblTouchStopper.visible = false;
    ...
    cntEmpty = new SMF.UI.Container({
            left : 0,
            top : 0,
            height : Device.screenHeight / 8,
            width : "100%"
        });
    cntEmpty.borderWidth = 0;

    var sasInfoFont = new SMF.UI.Font();
    sasInfoFont.bold = false;
    sasInfoFont.size = '5pt';

    var lblSasVersion = new SMF.UI.Label({
            left : 0,
            top : '93%',
            height : '7%',
            width : "100%",
    ...
    var imgLogo = new SMF.UI.Image({
            left : 0,
            top : '90%',
            height : '7%',
            width : "100%",
        });
    imgLogo.image = 'appstudiologo.png';
    imgLogo.imageFillType = SMF.UI.ImageFillType.aspectFit;
    var lblSasVersionText = new SMF.UI.Label({
            left : 0,
            top : '93%',
            height : '3%',
            width : "100%",
    ...
    lblSasVersion.backgroundTransparent = true;
    lblSasVersion.borderWidth = 0;
    lblSasVersion.font = sasInfoFont;
    lblSasVersion.fontColor = '#333333';
    lblSasVersion.touchEnabled = false;
    lblSasVersion.multipleLine = true;
    
    lblSasVersionText.backgroundTransparent = true;
    lblSasVersionText.borderWidth = 0;
    lblSasVersionText.font = sasInfoFont;
    lblSasVersionText.fontColor = '#333333';
    lblSasVersionText.touchEnabled = false;
    lblSasVersion.textAlignment = SMF.UI.TextAlignment.center;
    lblSasVersionText.textAlignment = SMF.UI.TextAlignment.center;

    Pages.pgBlog.add(rectBackground);
    //Pages.pgBlog.add(contCategories);
    sdLeftMenu.add(contCategories);
    sdLeftMenu.add(lblTouchStopper);
    ...
    /*if (Device.deviceOS == 'iOS') {
    contCategories.add(imgCategory6);
    contCategories.add(img7);
    } else {
    contCategories.add(cntEmpty);
    }*/
    contCategories.add(lblSasVersion);
    //contCategories.add(imgLogo);
    contCategories.add(lblSasVersionText);
}
var isCategory = 1;
...
function onSelectedItemBlog() {
    if (Device.connectionType != 0) {
        ...
        lblContentinScroll.text = textCon; //htmlReplace(textCon);
        if (Device.deviceOS == "Android") {
            headerSocial.init(page2);
            headerSocial.setLeftItem();
            headerSocial.setRightItem();
...
function infoPressed() {
    ...
    Device.share(titleUrl, urlShare + "\n\n" + lang.dlSmartface + "\n\n" + shareLink);
}
function onSyndicationSuccessBlog(e) {
    ...
    if (responseObjectBlog.posts.length > 0) {
        for (var i = 0; i < responseObjectBlog.posts.length; i++) {
            if (responseObjectBlog.posts[i].title_plain != "iOS 8 Jailbreak Released") {
