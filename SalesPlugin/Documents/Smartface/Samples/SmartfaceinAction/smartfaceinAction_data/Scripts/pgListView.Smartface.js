...
var globalScrollY = 0;
var tabCnt = Pages.pgListView.contTabHome;
var listViewRepeatBox = Pages.pgListView.rpbxLine;
/**
 * Creates action(s) that are run when the object is scrolling.
 * @param {EventArguments} e Returns some attributes about the specified functions
 * @this SMF.UI.RepeatBox
 */
function pgListView_rpbxLine_OnScroll(e) {
    /*if (e.scrollY > globalScrollY) {
        if ((tabCnt.top + tabCnt.height) > 0) {
            tabCnt.top -= e.scrollY - globalScrollY;
            //listViewRepeatBox.top -= e.scrollY - globalScrollY;
            //listViewRepeatBox.height += e.scrollY - globalScrollY;
            globalScrollY = e.scrollY;
        }
    } else {
        if (tabCnt.top < 0) {
            tabCnt.top += globalScrollY - e.scrollY;
            //listViewRepeatBox.top += e.scrollY - globalScrollY;
            //listViewRepeatBox.height -= e.scrollY - globalScrollY;
            globalScrollY = e.scrollY;
        }
    ...
    /*if(tabCnt.top > 0)
        tabCnt.top = 0;
    else if ((tabCnt.top + tabCnt.height) < 0)
        tabCnt.top = 0;*/
}